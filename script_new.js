...
function init () {
    ...
    canvasMouse.addEventListener('mousemove', event => {
        // запросим с начало позиция x, y через getBoundingclientRect
        const rect = canvasMouse.getBoundingClientRect()
        // Получаем координати непосредствено над самой Canvas
...
function loop () {
    ...
    drawLine(
        canvas.width / 2,
        canvas.height / 2,
        canvas.width / 2 + 200 * Math.cos(tick),
        canvas.height / 2 + 200 * Math.sin(tick),
        200,
        200, 
        "green", 
        10
